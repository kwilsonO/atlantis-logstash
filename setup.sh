...
if [ $# -ne 0 ]; then
	...
	sed -i.bak -E "s/LS_SUB_REGION=\".+?\"/LS_SUB_REGION=\"${OPTREGION}\"/g" $OPTPATH/atlantis.config
	sed -i.bak -E "s/LS_NODE_NAME=\"\"/LS_COMPONENT_TYPE=\"${OPTNAME}\"/g" $OPTPATH/atlantis.config
	sed -i.bak -E "s/LS_INDEX_PREFIX=\".+?\"/LS_COMPONENT_TYPE=\"${OPTNAME}\"/g" $OPTPATH/atlantis.config
